...
from ops import non_local_block
import time
    ...
    def __call__(self, z):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            net = tf.contrib.layers.batch_norm(net,
                    updates_collections=None, is_training=self._is_train, center=True, scale=True, decay=0.9, epsilon=1e-5, scope="g_bn_deconv0")
            net = tf.nn.relu(net)
            ...
            net = tf.contrib.layers.batch_norm(net,
                    updates_collections=None, is_training=self._is_train, center=True, scale=True, decay=0.9, epsilon=1e-5, scope="g_bn_deconv3")
            net = tf.nn.relu(net)
